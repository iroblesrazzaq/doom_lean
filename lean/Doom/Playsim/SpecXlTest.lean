...
/-!
P2c-xl unit checks: `T_PlatRaise` plat_down `downWaitUpStay` resultPastdest →
waiting + `sfx_pstop`, no `removeActivePlat`. Kept out of `EnemyTest.lean`
...
namespace Doom.Playsim.SpecXlTest
...
def checkP2cXlUnits (ok0 : Bool) : IO Bool := do
  let mut ok := ok0
      ...
      | some sec, some plat =>
        {
          ...
          sectors := GameState.arrSet gsDwus.sectors 1
            { sec with floorheight := plat.low + 1 }
        }
      | _, _ => gsDwus
    let rndBeforePast := gsNearDest.rng.rndindex
    match platRaiseThinker gsNearDest 0 with
    | Except.error e =>
      ok := (← assert s!"T_PlatRaise DOWN pastdest ({e})" false) && ok
    | Except.ok gsPast =>
      match gsPast.sectors[1]?, gsPast.plats[0]?, gsDwus.plats[0]? with
      | some sec, some plat, some plat0 =>
        ok := (← assert "T_PlatRaise DOWN pastdest floor snaps to low"
          (sec.floorheight == plat0.low)) && ok
        ok := (← assert "T_PlatRaise DOWN pastdest status=waiting count=wait"
          (plat.status == plat_waiting && plat.count == plat.wait
            && plat.wait == plat0.wait)) && ok
        ok := (← assert "T_PlatRaise DOWN pastdest specialdata unchanged"
          (sec.specialdata == 0)) && ok
      | _, _, _ =>
        ok := (← assert "T_PlatRaise DOWN pastdest sector/plat" false) && ok
      let mut foundPlatTh := false
      for th in gsPast.thinkers do
        if th.func == THF_PLATRAISE && th.payload == 0 then
          foundPlatTh := true
      ok := (← assert "T_PlatRaise DOWN pastdest THF_PLATRAISE retained" foundPlatTh) && ok
      ok := (← assert "T_PlatRaise DOWN pastdest activePlats[0]=0"
        (match gsPast.activePlats[0]? with
         | some s => s == 0
         | none => false)) && ok
      ok := (← assert "T_PlatRaise DOWN pastdest pstop rnd+1"
        (gsPast.rng.rndindex == rndBeforePast + 1)) && ok

  pure ok

end Doom.Playsim.SpecXlTest
