...
static void D_Endoom(void)
{
    ...
    // game has actually started. Skip under -tracedemo so exit does
    // not block waiting for a keypress (dummy SDL has no input).
...
void D_DoomMain (void)
{
    ...
    // Trace harness: force dummy SDL drivers before any subsystem init.
    OTrace_EarlyInit();
...
//    W_PrintDirectory();
    ...
    if (!p)
    {
        ...
        // @category demo
        //
        // Play a demo under the trace harness (lump name or .lmp path).
        //
    ...
    if (p)
    {
        ...
        else
        {
            ...
            // For -tracedemo, an existing file path is preferred; otherwise
            // the argument is treated as a WAD lump name (e.g. DEMO1).

            if (M_CheckParmWithArgs("-tracedemo", 1)
                && M_FileExists(myargv[M_CheckParmWithArgs("-tracedemo", 1) + 1]))
            {
                // Existing path that wasn't added as .lmp — try the raw path.
                if (D_AddFile(myargv[M_CheckParmWithArgs("-tracedemo", 1) + 1]))
                {
                    M_StringCopy(demolumpname, lumpinfo[numlumps - 1]->name,
                                 sizeof(demolumpname));
                }
