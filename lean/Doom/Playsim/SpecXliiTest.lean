...
/-!
P2c-xlii unit checks: vanilla `P_CrossSpecialLine` special 62 walk no-op.
Player walk returns identity (special stays 62, no plat, no thinker, no rng).
...
namespace Doom.Playsim.SpecXliiTest
...
def checkP2cXliiUnits (ok0 : Bool) : IO Bool := do
  let mut ok := ok0

  let gs62 := mkGs 0 62
  match Spec.crossSpecialLine gs62 0 0 0 with
  | Except.error e =>
    ok := (← assert s!"player walk spec 62 identity ({e})" false) && ok
  | Except.ok gs1 =>
    ok := (← assert "player walk spec 62 identity no plat/rng"
      (identityWalk gs62 gs1 62
        && gs1.plats.size == 0
        && gs1.verticalDoors.size == 0)) && ok

  let gsMon := { gs62 with mobjs := #[{ Mobj.empty with player := -1 }] }
  match Spec.crossSpecialLine gsMon 0 0 0 with
  | Except.error e =>
    ok := (← assert s!"monster walk spec 62 identity ({e})" false) && ok
  | Except.ok gsM =>
    ok := (← assert "monster walk spec 62 identity via !monsterOk"
      (identityWalk gsMon gsM 62)) && ok
