...
/-!
P2c-xxxvi unit checks: `EV_VerticalDoor` / `P_UseSpecialLine` special 27
(yellow DR). Keyed open matches special 1 (`vld_normal`, `sfx_doropn`, does
not clear `line.special`). No-key deny is `S_StartSound(NULL, sfx_oof)`
(always one pitch draw). Monster `useSpecialLine` 27 is false. Direct
...
namespace Doom.Playsim.SpecXxxviTest
...
def checkP2cXxxviUnits (ok0 : Bool) : IO Bool := do
  let mut ok := ok0

  let gsCard := setCard (idleGs 0 27) it_yellowcard
  match useSpecialLine gsCard 0 0 0 with
  | Except.error e =>
    ok := (← assert s!"yellowcard open ({e})" false) && ok
  | Except.ok (gs1, used) =>
    ok := (← assert "yellowcard UseSpecialLine used" used) && ok
    ok := (← assert "yellowcard open fields" (doorOpenOk gsCard gs1)) && ok
    match verticalDoorThinker gs1 0 with
    | Except.error e =>
      ok := (← assert s!"yellowcard same-tic UP ({e})" false) && ok
    | Except.ok gsUp =>
      match gsUp.sectors[0]? with
      | none => ok := (← assert "yellowcard UP sector" false) && ok
      | some sec =>
        ok := (← assert "yellowcard same-tic UP += VDOORSPEED"
          (sec.ceilingheight == 10 * FRACUNIT + VDOORSPEED)) && ok

  let gsSkull := setCard (idleGs 0 27) it_yellowskull
  match evVerticalDoor gsSkull 0 0 with
  | Except.error e =>
    ok := (← assert s!"yellowskull open ({e})" false) && ok
  | Except.ok gs1 =>
    ok := (← assert "yellowskull open fields" (doorOpenOk gsSkull gs1)) && ok

  let gsP1 := setCard (idleGs 1 27) it_yellowcard 1
  match evVerticalDoor gsP1 0 0 with
  | Except.error e =>
    ok := (← assert s!"player1 yellowcard open ({e})" false) && ok
  | Except.ok gs1 =>
    ok := (← assert "player1 yellowcard open fields" (doorOpenOk gsP1 gs1)) && ok
  let gsWrong := setCard (idleGs 1 27) it_yellowcard 0
  ...
  | Except.ok (gs1, used) =>
    ...
    ok := (← assert "no-key deny no thinker special stays 27"
      (gs1.verticalDoors.size == 0 && gs1.thinkers.size == gsNo.thinkers.size
        && (match gs1.level.lines[0]? with
            | some ld => ld.special == 27
            | none => false))) && ok

  let gsFar0 := idleGs 0 27
  let gsFar :=
  ...
  let gsMon := idleGs (-1) 27
  match useSpecialLine gsMon 0 0 0 with
  | Except.error e =>
    ok := (← assert s!"monster useSpecialLine 27 ({e})" false) && ok
  | Except.ok (gs1, used) =>
    ok := (← assert "monster useSpecialLine 27 false" (!used)) && ok
    ok := (← assert "monster useSpecialLine 27 identity"
  ...
  let gs1s :=
    let gs := idleGs 0 27
    { gs with level := { gs.level with lines := #[{ mkLine 27 with sidenum1 := -1 }] } }
  ...
  let gs1sKey := setCard gs1s it_yellowcard
  ...
  let gsClose := setCard (busyDoorGs 0 (-1) 27) it_yellowcard
  ...
  let gsRaise := setCard (busyDoorGs 0 1 27) it_yellowcard
  ...
  let gsMiss := { setCard (busyDoorGs 0 (-1) 27) it_yellowcard with verticalDoors := #[] }
  ...
  let gsPlat := {
    setCard (busyDoorGs 0 (-1) 27) it_yellowcard with
    verticalDoors := #[]
    plats := #[dummyPlat]
  }
...
end Doom.Playsim.SpecXxxviTest
