...
/-!
P2c-w unit checks: vanilla `P_CrossSpecialLine` special 26 walk no-op.
Player walk returns identity (special stays 26, no door, no plat, no thinker,
no rng). Does not call `EV_VerticalDoor`. Unhandled 88 and 5 stay loud-error.
Spec 1 walk no-op unchanged. Use-path special 26 unchanged (P2c-v).
...
namespace Doom.Playsim.SpecWTest
...
private def withBlue (gs : GameState) : GameState :=
  match gs.players[0]? with
  | none => gs
  | some p =>
    { gs with
      players := GameState.arrSet gs.players 0
        { p with cards := GameState.arrSet p.cards it_bluecard true } }
...
def checkP2cWUnits (ok0 : Bool) : IO Bool := do
  let mut ok := ok0

  let gs26 := mkGs 0 26
  match Spec.crossSpecialLine gs26 0 0 0 with
  | Except.error e =>
    ok := (← assert s!"player walk spec 26 identity ({e})" false) && ok
  | Except.ok gs1 =>
    ok := (← assert "player walk spec 26 identity no door/plat/rng"
      (identityWalk gs26 gs1 26
        && gs1.plats.size == 0
        && gs1.verticalDoors.size == 0)) && ok

  let gsKey := withBlue (mkGs 0 26)
  match Spec.crossSpecialLine gsKey 0 0 0 with
  | Except.error e =>
    ok := (← assert s!"keyed player walk spec 26 identity ({e})" false) && ok
  | Except.ok gs1 =>
    ok := (← assert "keyed player walk spec 26 still no EV_VerticalDoor"
      (identityWalk gsKey gs1 26
